...
- **이동 중 발견**: 게이지 급격히 상승
- **정지 + 위장 성공** (색상 거리 ≤ 110): 술래가 구별 못하고 지나감! 게이지 감소
- **정지 + 위장 실패** (색상 거리 > 110): 게이지가 빠르게 상승
...
방 안에 **7개의 동전**이 숨겨져 있습니다. 승리하려면 동전을 모두 수집하고 타이머가 끝날 때까지 생존해야 합니다.

- 동전은 방 구석, 장애물 근처 등 전략적 위치에 배치됩니다
- 플레이어가 동전 근처로 이동하면 **자동 수집**되며 효과음이 재생됩니다
- 좌측 HUD 패널에서 수집 현황(🪙 3/7)을 확인할 수 있습니다
- **모든 동전 수집 + 타이머 생존 = 승리**
- 동전을 다 모으지 못하고 타이머가 끝나면 **게임오버**
...
- `src/components/Coins.tsx`: 방 안에 배치된 수집 동전 3D 렌더링 (금색 원판 + 회전/떠다니는 애니메이션)
